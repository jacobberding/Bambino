...
namespace EDCCommands
{
    ...
    public static class ExtensionMethods
    {
        ...
        public static void LockOrUnlockAllLayers(this Document doc, bool isLocked, bool lockZero = false)
        {
            ...
            // These two calls will result in the layer's geometry fading/unfading appropriately
        ...
        public static void UsingLayerTable(this Database database, Action<IEnumerable<LayerTableRecord>> action)
        {
            database.UsingTransaction(
                tr => action(from ObjectId id in database.LayerTableId.OpenAs<LayerTable>(tr)
                             select id.OpenAs<LayerTableRecord>(tr).ForWrite()));
        }
        ...
        public static void GetEntity<T>(this Editor editor, string message, OpenMode openMode, Action<T> action)
            where T : Entity
        {
            RXClass rxClass = RXObject.GetClass(typeof(T));
