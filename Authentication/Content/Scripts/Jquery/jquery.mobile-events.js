...
(function ($) {
    ...
    // Add Event shortcuts:
    $.each(['tapstart', 'tapend', 'tapmove', 'tap', 'tap2', 'tap3', 'tap4', 'singletap', 'doubletap', 'taphold', 'swipe', 'swipeup', 'swiperight', 'swipedown', 'swipeleft', 'swipeend', 'scrollstart', 'scrollend', 'orientationchange', 'drag'], function (i, name) {
        $.fn[name] = function (fn) {
    ...
    $.event.special.swipe = {
        setup: function () {
            ...
            function touchMove(e) {
                ...
                if (swipedir != undefined && started) { // && (Date.now() - startEvnt.time) > 100
                    originalCoord.x = 0;
                    originalCoord.y = 0;
                    finalCoord.x = 0;
                    finalCoord.y = 0;
                    ...
                    hasSwiped = true;
                    //console.log(endEvnt.time - startEvnt.time);
                    $this.trigger('swipe', touchData).trigger(swipedir, touchData);
                }// else if (swipedir != undefined && started && (Date.now() - startEvnt.time) < 100) {
                   // started = false;
                   // //console.log('dragged');
                   // $this.trigger('drag', touchData);
                //}
            }
        ...
        setup: function () {
            ...
            $this.on(settings.startevent, function tapFunc1(e) {
                ...
                if (origTarget == e.target && started && ((Date.now() - start_time) < settings.taphold_threshold) && ((start_pos.x == end_x && start_pos.y == end_y) || (diff_x >= -(settings.tap_pixel_range) && diff_x <= settings.tap_pixel_range && diff_y >= -(settings.tap_pixel_range) && diff_y <= settings.tap_pixel_range))) {
                    ...
                    triggerCustomEvent(thisObject, 'drag', e, touchData);
                }
            });
        },
