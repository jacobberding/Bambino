...
const Disciplines = (function () {
    ...
    const _add = function () {
        ...
        const vm = {
            tableKey: ContactCompany.getSelf().vm.contactCompanyKey,
            name: $(`#txtSearch${_self.name}`).val()
        };

        Global.post(`ContactCompany_AddDiscipline`, vm)
            .done(function (data) {

                if ($(`m-card[data-id="${data.manyKey}"]`).length == 0)
    ...
    const _delete = function ($this) {
        ...
        Global.post(`ContactCompany_DeleteDiscipline`, vm)
            .done(function (data) {

                $(`m-card[data-id="${vm.manyKey}"]`).remove();
    ...
    const getHtmlTag = function (obj) {
        return `
            ...
            <m-card class="tag" data-id="${obj.disciplineKey}">
                <m-flex data-type="row" class="n">
                    <h1 class="tE">
                        ${obj.name}
                    </h1>
                    <m-flex data-type="row" class="n c xs sQ tertiary btnDelete${_self.name}" data-id="${obj.disciplineKey}">
